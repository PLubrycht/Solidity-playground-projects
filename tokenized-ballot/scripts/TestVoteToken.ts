...
async function main() {
  ...
  console.log(
    `Account ${
      acc1.account.address
    } has ${votesAfter.toString()} unit of voting power after self delegating\n`
  );
  ...
  console.log(
    `Account ${
      acc2.account.address
    } has ${votes2AfterTransfer.toString()} unit of voting power after receiving a transfer\n`
  );
  ...
  await publicClient.waitForTransactionReceipt({ hash: delegateTx });
  const votesAfterTransfer = await contract.read.getVotes([
    acc2.account.address,
  ]);
  console.log(
    `Account ${
      acc2.account.address
    } has ${votesAfterTransfer.toString()} unit of voting power after self delegating\n`
  );
